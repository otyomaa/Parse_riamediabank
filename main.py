...
def parse():
    ...
    # Путь к папке, куда будут сохранены изображения
    ...
    # Получение списка ссылок статей
    ...
    for a_tag in a_tags:
        ...
        a_page_soup = BeautifulSoup(requests.get(urljoin(base_url, a_tag.get("href")), headers=headers).text,
                                    "html.parser")
        # a_page = urljoin(base_url, a_tag.get("href"))
        # a_page_soup = BeautifulSoup(requests.get(a_page, headers=headers).text, "html.parser")

        a_tags_page = a_page_soup.find_all("a", class_="list-item__link")

        # Перебор тегов <a> со ссылками на изображение в статье
        for a_tag_page in a_tags_page:
            ...
            img_name = f"{'_'.join(h3_tag.string.split()[:30])}.jpg"  # Имя файла с расширением
